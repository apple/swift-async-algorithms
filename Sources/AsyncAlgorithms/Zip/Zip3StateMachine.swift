...
where Element1: Sendable, Element2: Sendable, Element3: Sendable {
  typealias SuspendedDemand = UnsafeContinuation<(Result<Element1, Error>, Result<Element2, Error>, Result<Element3, Error>)?, Never>
  ...
  mutating func newDemandFromConsumer(suspendedDemand: SuspendedDemand) -> NewDemandFromConsumerOutput {
    switch self.state {
      case .initial:
  ...
  mutating func newLoopFromBase1(suspendedBase: UnsafeContinuation<Void, Never>) -> NewLoopFromBaseOutput {
    switch self.state {
      ...
      case .awaitingDemandFromConsumer(let task, var suspendedBases):
        precondition(suspendedBases.count < 3, "There cannot be more than 3 suspended bases at the same time")
  ...
  mutating func demandIsFulfilled() {
    switch self.state {
      ...
      case .awaitingBaseResults(let task, let result1, let result2, let result3, let suspendedBases, let suspendedDemand):
        ...
        precondition(result1 != nil && result2 != nil && result3 != nil, "Inconsistent state, all results are not yet available to be acknowledged")
        self.state = .awaitingDemandFromConsumer(task: task, suspendedBases: suspendedBases)
