...
final class Zip2Runtime<Base1: AsyncSequence, Base2: AsyncSequence>: Sendable
where Base1: Sendable, Base1.Element: Sendable, Base2: Sendable, Base2.Element: Sendable {
  typealias ZipStateMachine = Zip2StateMachine<Base1.Element, Base2.Element>
  ...
  private func handle(baseHasProducedElementOutput: ZipStateMachine.BaseHasProducedElementOutput) {
    switch baseHasProducedElementOutput {
      ...
      case .resumeDemand(let suspendedDemand, let result1, let result2):
        suspendedDemand?.resume(returning: (result1, result2))
    }
  }
  ...
  private func handle(baseHasProducedFailureOutput: ZipStateMachine.BaseHasProducedFailureOutput) {
    switch baseHasProducedFailureOutput {
      ...
      case .resumeDemandAndTerminate(let task, let suspendedDemand, let suspendedBases, let result1, let result2):
        suspendedDemand?.resume(returning: (result1, result2))
        suspendedBases.forEach { $0.resume() }
        task?.cancel()
    }
