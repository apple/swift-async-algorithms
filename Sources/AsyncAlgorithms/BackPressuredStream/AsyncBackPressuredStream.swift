...
public struct AsyncBackPressuredStream<Element, Failure: Error>: AsyncSequence {
    /// A private class to give the ``AsyncBackPressuredStream`` a deinit so we
    /// can tell the producer when any potential consumer went away.
    private final class _Backing: Sendable {
        /// The underlying storage.
        fileprivate let storage: _AsyncBackPressuredStreamBackPressuredStorage<Element, Failure>

        init(storage: _AsyncBackPressuredStreamBackPressuredStorage<Element, Failure>) {
    ...
    public static func makeStream(
        ...
        let storage = _AsyncBackPressuredStreamBackPressuredStorage<Element, Failure>(
...
extension AsyncBackPressuredStream {
    ...
    public struct Source: Sendable {
        ...
        internal init(storage: _AsyncBackPressuredStreamBackPressuredStorage<Element, Failure>) {
        ...
        ///
        /// Call this method after ``write(contentsOf:)-4amlx`` or ``write(_:)-8e7el`` returned ``WriteResult/enqueueCallback(_:)``.
        ///
